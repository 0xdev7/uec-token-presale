...
contract TokenPresale is Ownable {
    ...
    constructor(UEDCoin _token) public {
        token = _token;
        
        withdrwAddress = 0x88501d955B56a4513F41E4E2A0cc6072645543f4;
        
        rewardTokenCount = 800000000000000;
        
        presaleStartTimestamp = now;
        presaleEndTimestamp = now.add(400 days);
    }
    ...
    function deposit() public payable {
        ...
        require(msg.value >= 10000000000000000 && msg.value <= 100000000000000000000, "incorrect amount");

        uint256 tokenAmount = msg.value.mul(1e18).div(rewardTokenCount);
        
        token.presale(msg.sender, tokenAmount);
        withdrwAddress.transfer(msg.value);
    ...
    function burn() external onlyOwner {
        require(now >= presaleEndTimestamp, "presale is still active");
        token.burn(address(token), token.balanceOf(address(token)));
    }
